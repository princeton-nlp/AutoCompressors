...
class AutoCompressorModel(LlamaForCausalLM):
    ...
    def __init__(self, config):
        ...
        if config.summary_length > 0:
            self.embed_summary = nn.Embedding(config.summary_length, config.hidden_size)
    ...
    def forward_segment(
        ...
        segment_embeds = torch.cat([softprompt, segment_embeds, placeholder_embeds], dim=1).to(torch.bfloat16)
